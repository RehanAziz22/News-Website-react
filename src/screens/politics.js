...
function Politics() {
    ...
    const getData = async () => {
        ...
        await axios.get("https://newsapi.org/v2/everything?q=politics&apiKey=03a4359182214c24b8201e021a4589e0")
            .then((success) => {
                setPoliticsNews(success.data.articles);
            })
            .catch((err) => {
                console.log(err);
            });
            ...
            <Container sx={{ display: { xs: "block", sm: "flex" } }}>
                <div style={{
                    ...
                    <div style={{ width: "fit-content", padding: "0", overflow: "hidden" }}>
                        {politicsNews.slice(0, visible).map(politicsCards)}
                        {visible < politicsNews.length && (<Button sx={{ alignSelf: "center" }} onClick={loadMore}><ArrowCircleRightIcon sx={{ fontSize: "50px", color: "black" }} /></Button>)}
